...
module.exports = class PortProxy {
  ...
  constructor({
    ...
    // Debug.enable(log.info.namespace)
    // Debug.enable(log.error.namespace)
  }
  ...
  async initSocket() {
    const socketServer = net.createServer(this.socketConnectionHandler.bind(this))
    socketServer.listen(this.config.socketPort, this.config.socketAddress)
